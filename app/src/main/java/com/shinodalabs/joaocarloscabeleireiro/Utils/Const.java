package com.shinodalabs.joaocarloscabeleireiro.Utils;

public class Const {

    public static final String RESULT = "result";
    public static final String ID = "id";
    public static final String NAME = "name";
    public static final String PRICE = "price";
    public static final String TIME = "time";
    public static final String IMAGE = "image";
    public static final String DATE_SCHEDULE = "date_schedule";
    public static final String DATE_SCHEDULED = "date_scheduled";
    public static final String ID_USER = "id_user";
    public static final String NAME_USER = "name_user";
    public static final String IMAGE_USER = "image_user";
    public static final String IMAGE_SERVICE = "image_service";
    public static final String NAME_SERVICE = "name_service";
    public static final String PRICE_SERVICE = "price_service";
    public static final String TIME_SCHEDULE = "time_schedule";
    public static final String ID_SCHEDULE = "id_schedule";
    public static final String ID_SERVICE = "id_service";
    public static final String PHONE_USER = "phone_user";

    public static final String URL_200 = "200";
    public static final String URL_404 = "404";
    public static final String URL_400 = "400";

    public static final int GOOGLE_SIGN_IN_CODE = 101;

    public static final String JOAO_WHATS = "https://api.whatsapp.com/send?phone=5551991863466";
    public static final String SHINODA_LABS_WHATS = "https://api.whatsapp.com/send?phone=5551996236798";
    public static final String SHINODA_LABS_SITE = "http://joaocarloscabeleireiro-com-br.umbler.net/";
    public static final String JOAO_PLAY_STORE = "joaocarloscabeleireiro";
    public static final String JOAO_FACEBOOK = "joaocarlos.souza.75457081";
    public static final String JOAO_INSTAGRAM = "jc_cabeleireiro_";
    public static final String JOAO_EMAIL = "jccabelereiro.app@gmail.com";

}
